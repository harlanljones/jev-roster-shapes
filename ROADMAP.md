...
Delivered: `/` opens a start screen whose first section is the interactive roster-and-shapes graphic (SVG diamond, eight position lanes + DH, per-player shape glyph from the analyst-labeled 8-shape rubric v1 in `docs/SHAPE_TAXONOMY.md` / `src/lib/shapes/taxonomy.ts`, click/keyboard selection, player detail, full table equivalent), followed by five 2026 storyline cards (baseline/A/B chips, digest, sources). Opening a card goes through the D-36 per-bundle public acknowledgment; the workspace (save/export/import, evidence, Swap/Move/keyboard/drag, restricted block) is unchanged except it opens the acknowledged bundle with a "← Storylines" return. The five `public` bundles in `src/lib/storylines/` (built by `spikes/mlb-2026/build.mjs` from observed 2026 MLB Stats API totals through 2026-09-20, overall mode, splits null, placeholder caps, costs disabled) replace the deleted `src/lib/fixtures/` golden + 2025 spike runtime copies; `docs/examples/comparison-v1.json` stays read-only as the historical v1 illustration. Schema 1.0, validator semantics, `deterministic-engine-v1`, and persistence versioning are unchanged; contract/engine/registry/integration tests were re-targeted at the storyline bundles with the corrected D-40 hand-derived table (Contreras 0.127580 after proper rounding), and every e2e journey now opens a storyline from the library first. Files: `src/lib/storylines/`, `src/lib/shapes/taxonomy.ts`, `src/lib/ui/ShapeGlyph.svelte`, `src/lib/app/LibraryPage.svelte`, `src/lib/app/RosterShapesGraphic.svelte`, `src/lib/app/HomePage.svelte`, `src/routes/+page.svelte`, `spikes/mlb-2026/`, `tests/storylines/registry.test.ts`, all retargeted contract/engine/integration/e2e specs, `README.md`, `docs/DECISIONS.md` (D-38–D-40), `docs/SHAPE_TAXONOMY.md`, `eslint.config.js` (`storylines`/`shapes` elements).
