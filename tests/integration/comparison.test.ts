...
describe('comparison integration journey', () => {
	it('calculates, edits, saves, reloads, exports, and reimports one comparison', async () => {
		const initial = clone(goldenBundle);
		...
		const allocation = candidate.allocations.find((item) => item.templateId === 'vs-left-starter')!;
		...
		expect(
			saved.current.bundle.results.find((result) => result.scenarioId === 'candidate-a')
				?.feasibility
		).toBe('incomplete');
		...
		expect(model.activeScenarioId).toBe('candidate-a');
		expect(model.scenarios).toHaveLength(3);
		expect(model.scenarios.find((scenario) => scenario.id === 'candidate-a')?.offense.status).toBe(
			'unavailable'
		);
		expect(model.scenarios.find((scenario) => scenario.id === 'candidate-a')?.unsaved).toBe(false);
