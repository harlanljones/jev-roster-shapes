...
import { getStoryline, storylineRegistry } from '../../src/lib/storylines/registry';
...
function bundle(slug: string): Bundle {
	const found = getStoryline(slug);
	if (!found) throw new Error(`storyline ${slug} is missing`);
	return structuredClone(found.bundle);
}
